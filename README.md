...
точность около 97% на отложенной выборке). Чтобы классифицировать ею изображения:

```bash
imagenn --load cnn <папка_с_изображениями> \
        --configs-dir models --weights-dir models --loss-dir models
```

Изображения должны быть рукописными цифрами в стиле MNIST (тёмная цифра на светлом
фоне или наоборот); на картинках другого стиля точность будет ниже.
