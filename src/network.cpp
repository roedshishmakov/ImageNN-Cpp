...
double NeuralNetwork::train(const std::vector<TrainingExample>& data, double speed, bool verbose,
    ...
    for (std::size_t index = 0; index < data.size(); ++index) {
        const std::vector<double>& input = data[index].first;
        const std::vector<double>& refs = data[index].second;

        run(input);

        Layer& output_layer = *layers_.back();
        const double loss = output_layer.get_loss(refs);
        loss_total += loss;
                ...
                for (auto& neuron : layer->neurons()) {
                    for (Link* link : neuron->links_input()) {
                        if (std::abs(link->weight_delta()) > clip_value) {
                            link->set_weight_delta(link->weight_delta() > 0 ? clip_value
                                                                            : -clip_value);
                        }
                    }
                }
